...
| **M7 — SQL engine** | TBD | **M7.1 landed (minimal `SELECT *` only).** `engine::sql::parse_select` recognises `SELECT * FROM <table>` (case-insensitive, optional whitespace and trailing `;`). Projection lists, WHERE clauses, ORDER BY, joins, aggregates, subqueries, and UDFs all return `AE_PARSE_ERROR`. ABI thunks: `AdsCreateSQLStatement`, `AdsCloseSQLStatement`, `AdsPrepareSQL`, `AdsExecuteSQL`, `AdsExecuteSQLDirect` — execution opens the referenced table (alias-resolving through any attached DD) and returns its handle as the cursor. **Pending:** full Advantage SQL grammar (lexer + AST + planner + executor), xBase UDFs (LEFT, SUBSTR, CTOD, ...), AEP host for stored procedures, triggers, INSERT / UPDATE / DELETE / CREATE TABLE. |
...
- **121 doctest cases / 847 assertions passing** on Windows / MSVC 2022 Release.
...
- **Live tags:** `m0-done`, `m1-done`, `m2-done`, `m3-done`, `m3.5-done`, `m3.6-partial`, `m3.7-partial`, `m3.7-closed`, `m4-partial`, `m5-partial`, `m5.1-partial`, `m5.2-partial`, `m5.3-partial`, `m6-partial`, `m7.1-partial`.
