...
| **M3 — Indexes** | [`2026-05-03-openads-m3-indexes.md`](docs/superpowers/plans/2026-05-03-openads-m3-indexes.md) | **Done.** NTX read + write + create end-to-end (single-leaf inserts; multi-level split is a follow-up). `Order` + `Scope` on `Table` with index-driven `goto_top` / `goto_bottom` / `skip` / `seek_key`. 15 ACE entry points (`AdsOpenIndex` / `AdsCreateIndex` / `AdsSeek` / `AdsSetScope` / etc.). `AdsPackTable` / `AdsZapTable` stubbed pending M4 memo store. `AdsSetAOF` accepts and reports `ADS_OPTIMIZED_NONE`. |
| **M3.5 — CDX index** | (extends M3) | **Done.** Real FoxPro-format CDX read+write: `CDXTAGHEADER` (1024 B) plus `CDXEXTNODE` compact leaves with bit-packed entries (24-bit recno + 8-bit dup + 8-bit trl), suffix bytes packed backward from page tail. `seek_first` / `seek_last` / `seek_key` / `next` / `prev` / `insert` / `erase` / `flush` / `create`. Single-leaf only for now (multi-leaf split deferred). L1 ABI auto-routes by `.cdx` / `.ntx` extension. |
