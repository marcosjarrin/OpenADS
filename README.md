...
| **M7 — SQL engine** | TBD | **M7.4 landed (`SELECT *` + single-comparison `WHERE` with all six operators).** `engine::sql::parse_select` parses `SELECT * FROM <table> [WHERE <col> <op> '<literal>']` where `<op>` is one of `=`, `!=`, `<>`, `<`, `>`, `<=`, `>=`. `Table` gained a `RowPredicate` slot: `goto_top` / `skip` automatically advance past non-matching records when a filter is attached. ABI: `AdsExecuteSQLDirect` compiles the WHERE into a closure that compares the named field's trimmed string value to the literal using the chosen operator. The full pipeline parses → opens table → installs filter → returns cursor; `AdsGotoTop` lands on the first match, `AdsSkip` walks subsequent matches, `AdsAtEOF` reports end after the last match. Projection lists, multi-clause WHERE (AND/OR/NOT), numeric literals, ORDER BY, joins, aggregates, subqueries, and UDFs return `AE_PARSE_ERROR`. **Pending:** full Advantage SQL grammar (lexer + AST + planner + executor), xBase UDFs (LEFT, SUBSTR, CTOD, ...), AEP host for stored procedures, triggers, INSERT / UPDATE / DELETE / CREATE TABLE. |
...
- **126 doctest cases / 890 assertions passing** on Windows / MSVC 2022 Release.
...
- **Live tags:** `m0-done`, `m1-done`, `m2-done`, `m3-done`, `m3.5-done`, `m3.6-partial`, `m3.7-partial`, `m3.7-closed`, `m4-partial`, `m5-partial`, `m5.1-partial`, `m5.2-partial`, `m5.3-partial`, `m6-partial`, `m7.1-partial`, `m7.2-partial`, `m7.3-partial`, `m7.4-partial`.
