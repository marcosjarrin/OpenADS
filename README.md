...
| **M5 — TPS / WAL** | TBD | **In-memory tx + WAL format landed.** `Connection::begin_tx` / `commit_tx` / `rollback_tx`, `AdsBeginTransaction` / `AdsCommitTransaction` / `AdsRollbackTransaction` / `AdsInTransaction` — Table writes capture pre-images during a tx; rollback restores them; append-then-rollback marks the new recno deleted. Persistent WAL file format (`engine::TxLog`) is implemented and tested: append-only records with `WAL\0` magic + type + length + tx_id + payload + CRC-32C; record types `BEGIN`, `UPDATE`, `COMMIT`, `ABORT`; corrupt-CRC truncates safely at the last valid record. **Pending:** wiring the WAL into `Connection` (write `BEGIN/UPDATE/COMMIT/ABORT` on tx events, store table path in records), three-phase ARIES-lite crash recovery on `Connection::open`, savepoints, group commit, page-LSN tracking with `.lsnmap` overlay for Compatible mode. |
