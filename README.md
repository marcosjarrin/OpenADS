...
| `m12.12-stub` | Phase 2 `tls://` URI scheme reservation — parser recognises `tls://host:port/<dir>` symmetrically to `tcp://`; `AdsConnect60` short-circuits to `AE_FUNCTION_NOT_AVAILABLE` (5004) with a clear "TLS not yet implemented" message so apps don't silently downgrade to plaintext. Real TLS transport (vendored mbedtls or platform-native) is the v0.4.0 milestone. |
