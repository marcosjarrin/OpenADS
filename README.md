...
| **M5 — TPS / WAL** | TBD | **In-memory tx + persistent WAL + crash recovery landed.** `Connection::begin_tx` / `commit_tx` / `rollback_tx`, `AdsBeginTransaction` / `AdsCommitTransaction` / `AdsRollbackTransaction` / `AdsInTransaction`. Each tx event writes a record to `openads.txlog` in the data dir (`BEGIN` / `UPDATE` / `COMMIT` / `ABORT`, all CRC-32C protected). UPDATE records carry the table relative path + before/after images. `Connection::open` runs recovery: any tx without `COMMIT` or `ABORT` is replayed by writing back its before-images and appending `ABORT`, then the log is truncated. Smoke test simulates a crash mid-tx and verifies the next connection restores the record byte-identical. **Pending:** savepoints, group commit, page-LSN tracking with `.lsnmap` overlay for Compatible mode. |
