...
| **M3.6 — Real index byte-compat** | TBD | Replace the M3.5 CDX layout with the real FoxPro compact format: keylen-derived bit widths, compound structure-tag directory, big-endian child pointers in branch nodes. Land NTX multi-level split and B-tree descent fixes. Tighten `AdsOpenIndex` lifecycle. Skip deleted records in `AdsCreateIndex`. Add round-trip tests for `descending` and `unique` flags. |
| **M4 — ADT + memo + VFP + AES** | [`2026-05-03-openads-m4-adt-memo-vfp-aes.md`](docs/superpowers/plans/2026-05-03-openads-m4-adt-memo-vfp-aes.md) | **In progress.** AES-128 / AES-256 ECB landed (vendored tiny-AES-c, validated against FIPS-197 + NIST SP 800-38A vectors). ADT driver with extended types, `.adm` / `.fpt` / `.dbt` memo, VFP driver still pending. |
