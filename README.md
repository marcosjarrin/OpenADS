...
| **M5 — TPS / WAL** | TBD | **In-memory tx landed.** `Connection::begin_tx` / `commit_tx` / `rollback_tx`, `AdsBeginTransaction` / `AdsCommitTransaction` / `AdsRollbackTransaction` / `AdsInTransaction` — Table writes capture pre-images during a tx; rollback restores them; append-then-rollback marks the new recno deleted. **Pending:** persistent WAL with three-phase ARIES-lite crash recovery, savepoints, group commit, page-LSN tracking with `.lsnmap` overlay for Compatible mode. |
