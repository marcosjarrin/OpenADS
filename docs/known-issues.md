...
All compat-breaking reviewer items now closed. Multi-tag-per-file (more than one sub-tag entry in the structure tag's leaf) remains a follow-up — the layout supports it; only `create()` and `open(path, mode, tag_name)` API plumbing is missing.
