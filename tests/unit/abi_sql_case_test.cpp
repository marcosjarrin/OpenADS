...
TEST_CASE("M10.38 SELECT CASE WHEN ... THEN ... ELSE ... END") {
    auto dir = fs::temp_directory_path() / "openads_m10_38";
    std::error_code ec;
    ...
    UNSIGNED8 sql[300] =
        ...
        "ELSE 'senior' END AS BUCKET FROM data.dbf";
    ADSHANDLE hCur = 0;
    ...
    auto bucket = [&](UNSIGNED32 r) {
        REQUIRE(AdsGotoRecord(hCur, r) == 0);
        UNSIGNED8 buf[40] = {0};
        UNSIGNED32 sz = sizeof(buf);
        REQUIRE(AdsGetField(hCur, (UNSIGNED8*)"BUCKET", buf, &sz, 0) == 0);
        std::string s((char*)buf, sz);
        while (!s.empty() && s.back() == ' ') s.pop_back();
        return s;
    };
    CHECK(bucket(1) == "adult");                     // Alice 25
    CHECK(bucket(2) == "adult");                     // Bob 35
    CHECK(bucket(3) == "senior");                    // Carol 50
    CHECK(bucket(4) == "minor");                     // Dan 17
