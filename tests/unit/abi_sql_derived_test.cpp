...
TEST_CASE("M10.46 derived table — FROM (SELECT ...)") {
    auto dir = fs::temp_directory_path() / "openads_m10_46";
    std::error_code ec;
    ...
    UNSIGNED8 sql[300] =
        "SELECT * FROM (SELECT * FROM data.dbf WHERE TAG > 'AAAA') AS T";
    ADSHANDLE hCur = 0;
    REQUIRE(AdsExecuteSQLDirect(hStmt, sql, &hCur) == 0);
    UNSIGNED32 cnt = 0;
    REQUIRE(AdsGetRecordCount(hCur, 0, &cnt) == 0);
    CHECK(cnt == 3);                              // BBBB, CCCC, DDDD
...
TEST_CASE("M10.48 CTE — WITH name AS (SELECT ...) SELECT ... FROM name") {
    auto dir = fs::temp_directory_path() / "openads_m10_48";
    std::error_code ec;
    fs::remove_all(dir, ec);
    fs::create_directories(dir);
    ...
    UNSIGNED8 sql[300] =
        "WITH BIG AS (SELECT * FROM data.dbf WHERE TAG > 'BBBB') "
        "SELECT * FROM BIG";
    ADSHANDLE hCur = 0;
    REQUIRE(AdsExecuteSQLDirect(hStmt, sql, &hCur) == 0);
    UNSIGNED32 cnt = 0;
    REQUIRE(AdsGetRecordCount(hCur, 0, &cnt) == 0);
    CHECK(cnt == 3);                              // CCCC, DDDD, EEEE
